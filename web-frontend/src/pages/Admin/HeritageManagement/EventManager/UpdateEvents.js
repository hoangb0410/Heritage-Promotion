...
const UpdateEvent = () => {
    ...
    useEffect(() => {
        const fetchEventId = async () => {
          try {
            const response = await axios.get(`${apiUrl}/v1/event/${params.id}`, {
              headers: {
                token: `Bearer ${accessToken}` 
              }
            });
            const eventData = response.data;
            eventData.event_date = formatDateToInputValue(eventData.event_date);
    ...
    const handleSubmit = async(event) => {
        ...
        await axios.put(`${apiUrl}/v1/event/update/${params.id}`, formData, {
            headers: {
              token: `Bearer ${accessToken}` 
            }
        });

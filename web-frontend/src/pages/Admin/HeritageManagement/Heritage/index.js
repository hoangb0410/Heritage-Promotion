...
function Heritage() {
  ...
  const totalHeritage = sites.length;
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const siteData = response.data.filter(site  => site.status === 'approved');
        setSites(siteData);
        ...
        <div className="">
          <div className="flex flex-wrap lg:space-x-10 mb-10">
            <div className="bg-white rounded-lg border-none p-5 shadow-xl max-w-72">
              <h2 className="text-xl font-semibold mb-2 flex justify-center">Tổng số di sản</h2>
              <div className="flex justify-center">
                <span className="h3 text-3xl">{totalHeritage}</span>
              </div>
              <Link to="create-heritage">
          ...
          <div className="overflow-x-auto bg-white rounded-lg shadow-xl border p-5">
            <div className="flex justify-between m-4">
              <h2 className="text-xl font-semibold mb-4">Danh sách di sản</h2>
              <input
            ...
            <table className="w-full table-auto">
              ...
              <tbody>
              { sites.length > 0 ? (filtereHeritages.map(site => (
                  <HeritageItem key={site._id} heritage={site} handleOpen={handleOpen} />
                ))) : (
                  <tr><td className="m-5">Chưa có di sản</td></tr>
              )}
              </tbody>
            </table>
          </div>
            ...
            <DialogBody divider>
              <div className="mb-5 w-full">
                <p className="my-2 text-[#000000]">
                  Bạn có chắc là muốn xóa di sản này không?
                  </p>
                </div>
            </DialogBody>
            <DialogFooter>

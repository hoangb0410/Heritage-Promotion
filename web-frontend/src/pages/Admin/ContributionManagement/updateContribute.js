...
function UpdateContribute() {
    ...
    // const isContentValid = formData.content.every(item => item.name.trim() !== '' && item.description.trim() !== '');
    ...
    const handleSubmit = async (event) => {
        ...
        const filteredContent = formData.content.filter(item => item.name.trim() !== '' && item.description.trim() !== '');
        ...
        navigate('/admin/contributemanagement');
    ...
    const handleAddContents = () => {
        // const filteredContent = formData.content.filter(item => item !== null);
        if (formData.content.some(item => item.name.trim() === '' || item.description.trim() === '')) {
            toast.warn('Vui lòng điền đầy đủ thông tin cho các phần tử content trước khi thêm mới.');
            return;
        }
        setFormData({
            ...formData,
    ...
    return ( 
        <>
            <div className="mb-2 mx-auto md:w-full m-10 border-none p-5">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-2">
                            ...
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    >
                                        <Link className="px-1 py-2" to="/admin/contributemanagement">Trở lại</Link>
                                    </div>
                                </div>
                                <div>
                                    <button
...
export default UpdateContribute;